...
export default function BlogsAdminPage() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || error.message || 'Failed to publish blog');
        } finally {
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-white p-4 md:p-8 pt-6 md:pt-12 overflow-x-hidden w-full max-w-8xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                ...
                {!isCreating && (
                    <button
                        onClick={() => { setIsCreating(true); setEditingId(null); form.reset({ title: "", author: "", content: "", imageUrl: "" }); }}
                            ...
                            <FormField control={form.control} name="content" render={({ field }) => (
                                <FormItem>
                                    <div className="flex items-center justify-between mb-1">
                                        <FormLabel className="text-xs font-medium text-zinc-300 block">Blog Content <span className="text-red-500">*</span></FormLabel>
                                ...
                                <tbody className="divide-y divide-zinc-800/50">
                                    {blogs.map((item) => (
                                        <tr key={item.id} className="hover:bg-zinc-800/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-zinc-400 text-sm hidden md:table-cell">{formatDate(item.createdAt)}</td>
