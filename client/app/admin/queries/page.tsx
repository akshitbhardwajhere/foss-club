...
import ApprovalDialog from "@/components/admin/queries/ApprovalDialog";
...
export default function QueriesAdminPage() {
  ...
  const [approveDialogData, setApproveDialogData] = useState<QueryRow | null>(
    null,
  );
  const [approveBasis, setApproveBasis] = useState<string>(
    ...
    <div className="min-h-screen bg-zinc-950 text-white p-4 md:p-8 pt-6 md:pt-12 overflow-x-hidden w-full max-w-8xl mx-auto">
      ...
      >
        <div className="overflow-x-auto min-h-75">
          {loading ? (
            <AdminTableSkeleton
              columns={["Contact Info", "Details", "Date", "Actions"]}
            />
          ) : queries.length === 0 ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                            ...
                            <div className="md:col-span-2 lg:col-span-3 xl:col-span-6 flex justify-end gap-2 mt-2">
                              ...
                              <button
                                ...
                                className="px-4 py-2 rounded-lg bg-[#08B74F] hover:bg-[#08B74F]/90 text-black font-bold text-sm"
                              >
                    ...
                    >
                      <td className="px-6 py-4">
                        <div>
                          ...
                          <a
                            href={`mailto:${q.email}`}
                            className="text-xs text-[#08B74F] hover:underline block mb-1"
                          >
                            {q.email}
                          </a>
                          ...
                          <div className="mt-2 text-left inline-block">
                            {q.status === "Approved" ? (
                              <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-[#08B74F]/20 text-[#08B74F] rounded-full">
                                Approved
                              </span>
                            ) : (
                              <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-orange-500/10 text-orange-400 border border-orange-500/20 rounded-full">
                                Pending
                              </span>
                            )}
                          </div>
      ...
      <ApprovalDialog
        ...
        target={
          approveDialogData
            ? { name: approveDialogData.name, email: approveDialogData.email }
            : null
        }
        basis={approveBasis}
        basisOptions={APPROVAL_BASIS_OPTIONS}
        isSubmitting={isApproving}
        onBasisChange={setApproveBasis}
        onClose={() => setApproveDialogData(null)}
