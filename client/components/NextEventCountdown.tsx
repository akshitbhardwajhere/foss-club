...
export default function NextEventCountdown() {
    ...
    const isLive = event && new Date(event.date).toDateString() === new Date().toDateString();
                            ...
                            >
                                <Link href={`/events/${event.id}`} className="block group w-full">
                                    <div className="flex flex-col items-center text-center w-full">
                                        <div className={`flex items-center justify-center gap-1.5 font-semibold text-[10px] 2xl:text-xs mb-1.5 sm:mb-2 uppercase tracking-wider px-3 py-1 rounded-full border ${isLive ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-[#08B74F]/10 text-[#08B74F] border-[#08B74F]/20'}`}>
                                            {isLive ? (
