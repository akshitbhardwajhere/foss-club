...
export function SortableTeamCard({ member, onEdit, onDelete }: SortableTeamCardProps) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`bg-zinc-900/50 border ${isDragging ? 'border-[#08B74F]' : 'border-zinc-800'} rounded-3xl p-5 pt-12 pb-5 flex flex-col items-center text-center relative group transition-colors`}
        >
            {/* Drag Handle */}
            <div
                {...attributes}
            ...
            <button
                ...
                className="absolute top-4 right-14 w-8 h-8 rounded-full bg-zinc-800 text-zinc-400 flex items-center justify-center hover:bg-zinc-700 hover:text-white transition-colors"
                title="Edit member"
            >
            ...
            <ConfirmDeleteDialog
                ...
                onConfirm={() => onDelete(member.id, member.name)}
            />
            ...
            <p className="text-xs text-[#08B74F] font-medium mb-3 bg-[#08B74F]/10 px-3 py-1 rounded-full">{member.role}</p>

            <div className="flex items-center justify-center gap-3 mt-auto pt-3 border-t border-zinc-800/50 w-full opacity-50">
                {member.githubUrl && <div className="rounded-full bg-zinc-700 p-2" title="GitHub linked">
                    <a href={ensureUrl(member.githubUrl)} target="_blank" rel="noopener noreferrer">
                        <Github className="w-4 h-4" />
                    </a>
                </div>}
                {member.linkedinUrl && <div className="rounded-full bg-blue-500/50 p-2" title="LinkedIn linked">
                    <a href={ensureUrl(member.linkedinUrl)} target="_blank" rel="noopener noreferrer">
                        <Linkedin className="w-4 h-4" />
                    </a>
                </div>}
                {member.twitterUrl && <div className="rounded-full bg-sky-500/50 p-2" title="Twitter linked">
                    <a href={ensureUrl(member.twitterUrl)} target="_blank" rel="noopener noreferrer">
